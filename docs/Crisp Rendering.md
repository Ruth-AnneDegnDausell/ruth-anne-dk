> **Opdateret:** 2026-04-23 — Alle font-referencer opdateret til Satoshi.
> Gamle fonte (JetBrains Mono, Space Grotesk, Inter, Sentient) er fjernet fra systemet.
> Se `packages/ui/src/tokens.ts` for kanoniske font-tokens.
...
With `optional`, the browser gives the font ~100ms to load. If it misses that window, the fallback renders for the entire page lifecycle — no swap, no layout shift, ever. Combined with `<link rel="preload">` for Satoshi, it's virtually guaranteed to load in time.
...
@font-face {
  font-family: 'Satoshi';
  src: url('/fonts/satoshi-latin.woff2') format('woff2');
...
@font-face {
  font-family: 'Satoshi';
  src: url('/fonts/satoshi-latin-ext.woff2') format('woff2');
  unicode-range: U+0100-024F, U+0259, U+1E00-1EFF, U+20A0-20AB;
}
```

The browser only downloads font files if characters from that unicode-range actually exist on the page. This is free performance — a full Satoshi is ~300KB, a subset for Western European with specific features can be under 50KB.
...
<!-- RAMTT: use WEIGHT.strong (550) — never font-bold/700 -->
<h1 class="text-7xl/[1.05] tracking-tighter font-[550]">

<!-- Section heading: 36px with 1.15 line-height -->
<!-- RAMTT: use WEIGHT.strong (550) — never font-semibold/600 -->
<h2 class="text-4xl/[1.15] tracking-tight font-[550]">
...
A full Satoshi is ~300KB. A subsetted version for Western European with specific features can be under 50KB.
